...
generate: configure-sqlc
	.gen/bin/sqlc generate
...
configure-sqlc:
	...
	mkdir .gen/bin -p
	# Get sqlc...
	which .gen/bin/sqlc || (curl -fsSL https://github.com/sqlc-dev/sqlc/releases/download/v1.29.0/sqlc_1.29.0_linux_amd64.zip -o .gen/bin/sqlc.zip && \
	unzip -j .gen/bin/sqlc.zip -d .gen/bin && rm .gen/bin/sqlc.zip -f)
	chmod +x .gen/bin/sqlc

configure: configure-bun configure-air configure-sqlc
