...
func GetLogin(c *frizzante.Connection) {
	c.SendView(frizzante.View{Name: "Login"})
}

func PostLogin(c *frizzante.Connection) {
	lib.SessionStartPublic(c, func(state *lib.State) {
		...
		if nil != accountError {
			c.SendView(frizzante.View{Name: "Login", Error: errors.New("invalid credentials")})
			return
		}
