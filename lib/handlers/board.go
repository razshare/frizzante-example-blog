...
func GetBoard(c *frz.Connection) {
	...
	if nil != articleError {
		c.SendView(frz.View{Name: "Board", Error: articleError})
		return
	}
...
func PostBoard(c *frz.Connection) {
	...
	if nil != articleIdError {
		c.SendView(frz.View{Name: "Board", Error: articleIdError})
		return
	}
	...
	if nil != addArticleError {
		c.SendView(frz.View{Name: "Board", Error: addArticleError})
		return
	}

	articleContentId, articleContentIdError := uuid.NewV4()
	if nil != articleContentIdError {
		c.SendView(frz.View{Name: "Board", Error: articleContentIdError})
		return
	}
	...
	if nil != addContentError {
		c.SendView(frz.View{Name: "Board", Error: addContentError})
		return
	}
